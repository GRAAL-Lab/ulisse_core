...
StoneFishVisualizer::StoneFishVisualizer(const std::string file_name)
...

{
    ...
    navDataSub_ = this->create_subscription<ulisse_msgs::msg::NavFilterData>(ulisse_msgs::topicnames::nav_filter_data,1, std::bind(&StoneFishVisualizer::NavDataCB, this, _1));
    simulatedSysSub_ = this->create_subscription<ulisse_msgs::msg::SimulatedSystem>(ulisse_msgs::topicnames::simulated_system,1, std::bind(&StoneFishVisualizer::SimSystemCB, this, _1));
    //visualizationPub_ = this->create_publisher<visualization_msgs::msg::Marker> ("visualization_marker", 0 );
    ulissePosePub_ = this->create_publisher<nav_msgs::msg::Odometry> ("/sf/ulisse/pose", 1 );
    ...
    ObstacleSub_ = this->create_subscription<ulisse_msgs::msg::Obstacle>(ulisse_msgs::topicnames::obstacle, 10,
                                                                                   std::bind(&StoneFishVisualizer::ObstacleCB, this, _1));
    ...
    runTimer_ = this->create_wall_timer(std::chrono::milliseconds(msRunPeriod), std::bind(&StoneFishVisualizer::Run, this));
    std::cout << "set time " << "\n";

}

bool StoneFishVisualizer::LoadConfiguration(const std::string file_name)
{
...
void StoneFishVisualizer::ExecuteStep()
{
...
void StoneFishVisualizer::AssignMessage(std::array<double,6>& msg,const Eigen::Vector6d& vector){
    for(unsigned long i=0; i < msg.size(); i++){
        msg[i] = vector(i);
    }
}

double StoneFishVisualizer::GetCurrentTimeStamp() const
{
    long now_nanosecs = (std::chrono::duration_cast<std::chrono::nanoseconds>(t_now_.time_since_epoch())).count();
    return static_cast<double>(now_nanosecs / 1E9);
}

void StoneFishVisualizer::VisualizeASV(){
    ...
    asvNavQ_.setEuler(bodyF_ASV_.Yaw(),bodyF_ASV_.Pitch(),bodyF_ASV_.Roll());
...
void StoneFishVisualizer::Visualize(){
    ulissePose_.header.stamp = this->get_clock()->now();

    asvNavLatLong_.latitude = navData_.inertialframe_linear_position.latlong.latitude;
    asvNavLatLong_.longitude = navData_.inertialframe_linear_position.latlong.longitude;
    ctb::LatLong2LocalUTM(asvNavLatLong_, navData_.inertialframe_linear_position.altitude, centroidLocation_, asvNavUTM_);
    ulissePose_.pose.pose.position.x = asvNavUTM_.y(); // inverted
    ulissePose_.pose.pose.position.y = asvNavUTM_.x(); // inverted
    ulissePose_.pose.pose.position.z = asvNavUTM_.z();

    tf2::Quaternion ulisse_q_;
    ulisse_q_.setEuler(bodyF_ASV_.Yaw(),bodyF_ASV_.Pitch(),bodyF_ASV_.Roll());
    ulissePose_.pose.pose.orientation.x = ulisse_q_.x();
    ulissePose_.pose.pose.orientation.y = ulisse_q_.y();
    ulissePose_.pose.pose.orientation.z = ulisse_q_.z();
    ulissePose_.pose.pose.orientation.w = ulisse_q_.w();

    ulissePosePub_->publish(ulissePose_);
}

void StoneFishVisualizer::UpdateFrames(){
    ...
    worldF_ASV_F_ = Ry_n * Rz_n* worldF_R_bodyF_  ;
    //bodyF_ASV_ = worldF_ASV_F_.eulerAngles(2, 1, 0);
    bodyF_ASV_ = worldF_R_bodyF_.eulerAngles(2, 1, 0);
}

void StoneFishVisualizer::VisualizeObstacles(){
...
/*void StoneFishVisualizer::PublishTf(){
...
/*void StoneFishVisualizer::PublishMarkerArray(){
    visualizationArrayPub_->publish(MarkerArray_);
    MarkerArray_.markers.clear();
}*/

void StoneFishVisualizer::NavDataCB(const ulisse_msgs::msg::NavFilterData::SharedPtr msg) { navData_ = *msg; }

void StoneFishVisualizer::SimSystemCB(const ulisse_msgs::msg::SimulatedSystem::SharedPtr msg) { simData_ = *msg; }

void StoneFishVisualizer::ObstacleCB(const ulisse_msgs::msg::Obstacle::SharedPtr msg){
    obstacle_.id = msg->id;
