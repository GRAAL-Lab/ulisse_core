...
def main(args=None):
    ...
    plt.figure()
    plt.subplot(221)
    plt.title("Surge refernece")
    plt.ylabel("surge refernece [m/s] ")
    plt.xlabel("time [s]")
    plt.plot(e['time'], e['surge_error'], 'r-')
    plt.grid(True)
    plt.subplot(222)
    plt.title("YawRate refernece")
    plt.ylabel("YawRate refernece [rad/s] ")
    plt.xlabel("time [s]")
    plt.plot(e['time'], e['yawr_error'], 'b-')
    plt.grid(True)
    plt.subplot(223)
    plt.title("Surge output")
    plt.ylabel("Surge output [m/s]")
    plt.xlabel("time [s]")
    plt.plot(e['time'], e['surge_control'], 'k-')
    plt.grid(True)
    plt.subplot(224)
    plt.title("YawRate output")
    plt.ylabel("YawRate output [rad/s]")
    plt.xlabel("time [s]")
