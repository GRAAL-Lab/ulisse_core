...
StatePathFollowILOSCurrent::StatePathFollowILOSCurrent()
    : isCurveSet_ { false }
...
bool StatePathFollowILOSCurrent::LoadPath(const ulisse_msgs::msg::PathData& path){
    vehicleOnTrack_ = false;
...
bool StatePathFollowILOSCurrent::ConfigureStateFromFile(libconfig::Config& confObj)
{
    const libconfig::Setting& root = confObj.getRoot();
    const libconfig::Setting& states = root["states"];

    const libconfig::Setting& state = states.lookup(ulisse::states::ID::pathfollow_iloscurrent);
    ...
    if (!pathManager_.nurbsParam.configureFromFile(confObj, ulisse::states::ID::pathfollow_iloscurrent)) {
...
fsm::retval StatePathFollowILOSCurrent::OnEntry()
{
    std::cout << "************* ILOS and LOS with Current estimator starts ***************" << std::endl;
    ...
    if (actionManager->SetAction(ulisse::action::pathfollow_iloscurrent, true)) {
        return fsm::ok;
    } else {
        return fsm::fail;
    }
...
fsm::retval StatePathFollowILOSCurrent::OnExit(){

    std::cout << "************* ILOS and LOS with current estimator finished ***************" << std::endl;
...
fsm::retval StatePathFollowILOSCurrent::Execute()
{
    ...
    if (isCurveSet_) {
        ...
        if (!vehicleOnTrack_) {
            ...
            if (pathManager_.DistanceToEnd() < tolleranceEndingPoint_) {
                ...
                bool pathmode;
                if (!pathManager_.ComputeGoalPosition(ctrlData->inertialF_linearPosition, nextP_,delta_y_,closePoint2path,pathmode)) {
                    return fsm::fail;
                }

                ctb::DistanceAndAzimuthRad(ctrlData->inertialF_linearPosition, nextP_, goalDistance, goalHeading);

                if(pathmode){ // LOS with current estimator mode
                    ...
                    y_ = INFO.y_; // in order to be published
                    LOS_goalHeading = ILOS_goalHeading;// in order to be published
