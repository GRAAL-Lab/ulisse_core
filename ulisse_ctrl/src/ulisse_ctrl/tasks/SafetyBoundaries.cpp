...
void SafetyBoundaries::ComputeAlignVector(segment_type segment,
  ...
  point_type p1{boost::geometry::get<0, 0>(segment),
                boost::geometry::get<0, 1>(segment)};

  // ending point of the first segment
  point_type p2{boost::geometry::get<1, 0>(segment),
                boost::geometry::get<1, 1>(segment)};
...
void SafetyBoundaries::ComputeNormalVector2Segment(
    segment_type segment, Eigen::Vector3d &alignVector) {
  ...
  // find a point on the line perpedicular to the segment and passing
  // through the mid point to check if the point is insede the polygon
  double k = 0.5;

  point_type p{midP.x() + k * uPerp.x(), midP.y() + k * uPerp.y()};

  // if not take the oder direction
  if (!boost::geometry::covered_by(p, poly_)) {
    uPerp.set<0>(u.x());
    uPerp.set<1>(-u.y());
  }
...
void SafetyBoundaries::ExtractMinDistanceSegments(
    std::list<segment_type> originalSegments, point_type currentPosition,
  ...
  for (std::list<segment_type>::iterator it = originalSegments.begin();
       it != originalSegments.end(); it++) {
...
bool SafetyBoundaries::IsConvex(std::list<segment_type> segments) {
  ...
  // starting point of the first segment
  point_type p1{boost::geometry::get<0, 0>(segments.front()),
                boost::geometry::get<0, 1>(segments.front())};
  //  // ending point of the segment
  point_type p2{boost::geometry::get<1, 0>(segments.front()),
                boost::geometry::get<1, 1>(segments.front())};

  point_type s1{boost::geometry::get<0, 0>(segments.back()),
                boost::geometry::get<0, 1>(segments.back())};

  // ending point of the segment
  point_type s2{boost::geometry::get<1, 0>(segments.back()),
                boost::geometry::get<1, 1>(segments.back())};
