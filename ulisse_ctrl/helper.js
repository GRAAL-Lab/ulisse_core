...
function geo_midpoint(c1, c2){
    var Ï†1 = deg_to_rad(c1.latitude)
    var Î»1 = deg_to_rad(c1.longitude)
    var Ï†2 = deg_to_rad(c2.latitude)
    var Î»2 = deg_to_rad(c2.longitude)

    var Bx = Math.cos(Ï†2) * Math.cos(Î»2-Î»1)
    var By = Math.cos(Ï†2) * Math.sin(Î»2-Î»1)
    var Ï†3 = Math.atan2(Math.sin(Ï†1) + Math.sin(Ï†2),
                        Math.sqrt( (Math.cos(Ï†1)+Bx)*(Math.cos(Ï†1)+Bx) + By*By ) )
    var Î»3 = Î»1 + Math.atan2(By, Math.cos(Ï†1) + Bx)
    return QtPositioning.coordinate(rad_to_deg(Ï†3),
                                    rad_to_deg((Î»3+540)%360-180))
}