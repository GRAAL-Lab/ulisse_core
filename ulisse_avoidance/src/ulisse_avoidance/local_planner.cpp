    ...
    std::shared_ptr<ulisse_msgs::srv::ComputeAvoidancePath::Response> response)
{
    ...
    if (plan->Report().result == oal::SearchResult::FAIL) {
        ...
        //HoldCmd();
        response->res = false;
        response->info ="path computation failed";
        return;
    }
    ...
    if (plan->Report().result == oal::SearchResult::PARTIAL){
        ...
        response->res = false;
        response->info ="Cannot reach goal";
    }
    ...
    PathCmd();
    response->path = currentPathMsg.path;
    response->res = true;
    response->info ="path generated";
...
bool LocalPlannerNode::PathCmd()
{
    ...
    RCLCPP_INFO(this->get_logger(), "Path generated and published!");
    //serviceReq->command_type = ulisse::commands::ID::pathfollow;
    //serviceReq->path_cmd = currentPathMsg;
    GuiPathPub();
    StartCheckingProgress();
    RCLCPP_INFO(this->get_logger(), "End of StartCheckingProgress()");
    //return WaitForResponse(serviceReq);
...
{/*
    ...
    if (kclCmdSrv_->service_is_ready()) {
        ...
        if (status != std::future_status::ready) { // commented by juriii
...
void LocalPlannerNode::TopicSetup()
{
    ...
    // KCL cmd service client (commented out by juri)
