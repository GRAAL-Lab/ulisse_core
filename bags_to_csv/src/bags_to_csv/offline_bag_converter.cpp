...
bool OfflineBagConverter::ConvertToCSV()
{
    ...
    while (reader.has_next()) {
        ...
        else if (bag_message->topic_name == ulisse_msgs::topicnames::sliding_mode_control) {
            rclcpp::Serialization<ulisse_msgs::msg::STSMControl> serialization;
            serialization.deserialize_message(&extracted_serialized_msg, &stsmControl_);
            stsmFile_ << std::fixed << std::setprecision(6)
                      << bag_message->time_stamp  * 1e-9 << ", "
                      << stsmControl_.stamp.sec + (stsmControl_.stamp.nanosec * 1e-9) << ", "
                      << stsmControl_.sigma_surge << ", " << stsmControl_.sigma_yawrate << ", "
                      << stsmControl_.tau_surge << ", " << stsmControl_.tau_yawrate << "\n";
        }
...
bool OfflineBagConverter::OpenFiles()
{
    ...
    stsmFile_         .open(std::string(saveFolder_ + "/stsm_control.txt"));
    stsmFile_        << "ros_time, time, sigma_surge, sigma_yawrate, tau_surge, tau_yawrate\n";
