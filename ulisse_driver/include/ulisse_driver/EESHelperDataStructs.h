...
/*
    ...
    public:
        ...
        ReferencesContainer();
        ReferencesContainer(const ReferencesContainer& rhs);

    protected:
        void Describe();
    };

    class SensorsContainer : public ortos::xcom::BaseContainer {
    public:
        ortos::ortostime_t timestamp;
        sensorData d;

        SensorsContainer();
        SensorsContainer(const SensorsContainer& rhs);

    protected:
        void Describe();
    };

    class StatusContainer : public ortos::xcom::BaseContainer {
    public:
        ortos::ortostime_t timestamp;
        statusData d;

        StatusContainer();
        StatusContainer(const StatusContainer& rhs);

    protected:
        void Describe();
    };

    class ConfigContainer : public ortos::xcom::BaseContainer {
    public:
        ortos::ortostime_t timestamp;
        configData d;

        ConfigContainer();
        ConfigContainer(const ConfigContainer& rhs);

    protected:
        void Describe();
    };

    class AckContainer : public ortos::xcom::BaseContainer {
    public:
        ortos::ortostime_t timestamp;
        ackData d;

        AckContainer();
        AckContainer(const AckContainer& rhs);

    protected:
        void Describe();
    };

    class VersionContainer : public ortos::xcom::BaseContainer {
    public:
        ortos::ortostime_t timestamp;
        versionData d;

        VersionContainer();
        VersionContainer(const VersionContainer& rhs);

    protected:
        void Describe();
    };

    class MotorsContainer : public ortos::xcom::BaseContainer {
    public:
        ortos::ortostime_t timestamp;
        motorsData d;

        MotorsContainer();
        MotorsContainer(const MotorsContainer& rhs);

    protected:
        void Describe();
    };

    class BatteryContainer : public ortos::xcom::BaseContainer {
    public:
        ortos::ortostime_t timestamp;
        batteriesData d;

        BatteryContainer();
        BatteryContainer(const BatteryContainer& rhs);

    protected:
        void Describe();
    };

    class Sw485StatusContainer : public ortos::xcom::BaseContainer {
    public:
        ortos::ortostime_t timestamp;
        sw485StatusData d;

        Sw485StatusContainer();
        Sw485StatusContainer(const Sw485StatusContainer& rhs);
    ...
    class CommandContainer : public om2ctrl::utils::BaseCommandContainer {
    public:
        CommandData d;
        ...
        virtual void Serialize(ortos::xcom::BaseMarshaller& marshaller) const;
        virtual void Deserialize(ortos::xcom::BaseMarshaller& marshaller);

        void DebugPrint(const char* string) {
		ortos::DebugConsole::Write(ortos::LogLevel::info, string, "Command \"%s\" generated by %s", CommandTypeToString(commandType).c_str(), producerTaskName);

		switch (commandType) {
			case (uint16_t)CommandType::undefined:
				break;
			case (uint16_t)CommandType::beep:
				d.beep.DebugPrint(string);
				break;
			case (uint16_t)CommandType::enableref:
				d.enableRef.DebugPrint(string);
				break;
			case (uint16_t)CommandType::setconfig:
				d.setConfig.DebugPrint(string);
				break;
			case (uint16_t)CommandType::setpumps:
				d.pumps.DebugPrint(string);
				break;
			case (uint16_t)CommandType::setpowerbuttons:
				d.powerButtons.DebugPrint(string);
				break;
			...
			case (uint16_t)CommandType::reloadconfig:
				break;
			default:
				ortos::DebugConsole::Write(ortos::LogLevel::info, string, "No DebugPrint for this command");
				break;
		}
    }
    ...
    protected:
        void Describe();
        void DescribeAndFinalize();
    };

    class CommandAnswerContainer : public om2ctrl::utils::BaseCommandAnswerContainer {
    public:
        CommandAnswerContainer();
        CommandAnswerContainer(const CommandAnswerContainer& rhs);

        void DebugPrint(const char* string) {
		ortos::DebugConsole::Write(ortos::LogLevel::info, string, "Answer for command \"%s\" generated by %s", CommandTypeToString(commandType_).c_str(), producerTaskName_);
		ortos::DebugConsole::Write(ortos::LogLevel::info, string, "commandTimestamp %lld - Answer: %s", commandTimestamp_, CommandAnswerToString(answer).c_str());
    }
    protected:
        void DescribeAndFinalize();
    };
